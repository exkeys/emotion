...
export async function analyzeHandler(req, res) {
  try {
    ...
    if (!req.body || typeof req.body !== 'object') {
      return res.status(400).json({ error: 'Invalid request: body must be JSON object' });
    }

    const { range, from, to, user_id = 'test_user' } = req.body;
    const hasRange = typeof range === 'string' && range.trim();
    const hasFromTo = typeof from === 'string' && typeof to === 'string';

    if (hasRange && hasFromTo) {
      return res.status(400).json({ error: 'Provide either range OR from/to dates, not both' });
    }
    if (!hasRange && !hasFromTo) {
      return res.status(400).json({ error: 'Missing parameters: provide range OR from/to' });
    }
    ...
    if (hasRange) {
      switch (range) {
        case 'daily': fromDate = now.startOf('day').format('YYYY-MM-DD'); break;
        case 'weekly': fromDate = now.startOf('week').format('YYYY-MM-DD'); break;
        case 'monthly': fromDate = now.startOf('month').format('YYYY-MM-DD'); break;
        default:
          return res.status(400).json({ error: 'Invalid range', allowed: ['daily','weekly','monthly'] });
      }
    ...
    console.log(`[분석] 사용자: ${user_id}, 기간: ${fromDate} ~ ${toDate}`);
    ...
    try {
      ...
      if (redis) {
        ...
        if (cachedJson) {
          return res.json({ result: JSON.parse(cachedJson), cached: true });
        }
      } else {
    ...
    if (!data || data.length === 0) {
      return res.json({ result: '해당 기간에 데이터가 없습니다. 먼저 기록을 추가해주세요.' });
    }

    const formatted = formatDataForAnalysis(data);

    console.log('[분석] OpenAI 요청 시작');
    ...
    try {
      ...
      console.warn('[분석] 1차 모델 지연/실패, 폴백 시도:', primaryErr?.message || primaryErr);
      try {
        ...
        console.error('[분석] 폴백 모델도 실패:', fallbackErr?.message || fallbackErr);
        return res.status(500).json({ error: 'AI 분석 실패', details: fallbackErr?.message || primaryErr?.message });
      }
    }
    ...
    res.json({ result: aiResponse, cached: false });
